...
# `NSEBaseMPIExt/src/abstractgrid.jl`.
#
# Runs under any rank count; uses a `MockChannelGrid` parent wrapped via
# `NSEBaseMPIExt.distributed` along the wall-normal `:y` direction. Every
...
Test.@testset "DecomposedGrid alias" begin
    Test.@test g isa NSEBaseMPIExt.DecomposedGrid
end
...
Test.@testset "comm / comm_size / comm_rank" begin
    ...
    g_comm = NSEBaseMPIExt.comm(g)
    Test.@test MPI.Cartdim_get(g_comm) == length(size(g))
    Test.@test NSEBaseMPIExt.comm_size(g) == nranks
    Test.@test NSEBaseMPIExt.comm_rank(g) == rank
end

Test.@testset "decomposition_storage_dims / _physical_dims" begin
    Test.@test NSEBaseMPIExt.decomposition_storage_dims(g) == (1,)
    Test.@test NSEBaseMPIExt.decomposition_physical_dims(g) == (:y,)
    Test.@test NSEBaseMPIExt.ndecomposed_dims(g) == 1
end
...
Test.@testset "global_first_index" begin
    Test.@test NSEBaseMPIExt.global_first_index(g, :y) == y_offset + 1
    # Non-decomposed coordinates always start at global index 1.
    Test.@test NSEBaseMPIExt.global_first_index(g, :x) == 1
    Test.@test NSEBaseMPIExt.global_first_index(g, :z) == 1
    Test.@test NSEBaseMPIExt.global_first_index(g, :t) == 1
end

Test.@testset "local_interior_range / local_boundary_ranges" begin
    interior = NSEBaseMPIExt.local_interior_range(g, :y)
    lower, upper = NSEBaseMPIExt.local_boundary_ranges(g, :y)
    ...
    Test.@test NSEBaseMPIExt.local_interior_range(g, :x) == 1:Nx
    Test.@test NSEBaseMPIExt.local_boundary_ranges(g, :x) == (1:0, 1:0)
end
...
Test.@testset "derivative_matrix forwards to parent" begin
    ...
    D1 = NSEBaseMPIExt.derivative_matrix(g, y_sd, Val(1))
    D2 = NSEBaseMPIExt.derivative_matrix(g, y_sd, Val(2))
    Test.@test D1 === g_parent.D₁
    Test.@test D2 === g_parent.D₂

    # Adjoint flag is forwarded.
    D1_adj = NSEBaseMPIExt.derivative_matrix(g, y_sd, Val(1), Val(true))
    Test.@test D1_adj == LinearAlgebra.adjoint(g_parent.D₁)

    # FFT directions have no parent FD operator; the parent throws.
    Test.@test_throws ArgumentError NSEBaseMPIExt.derivative_matrix(g, x_sd, Val(1))
end

Test.@testset "local_size names the per-rank interior size" begin
    Test.@test NSEBaseMPIExt.local_size(g) == size(g)
end
