...
@testset verbose=true "Norms                                                       " begin
    ...
    @testset verbose=true "FTField: dot symmetry, positivity, linearity                " begin
        # The parent array is `(Nx, (Ny>>1)+1)` and the first dimension uses
        # the quadrature supplied by the rectangular grid.
        Nx, Ny = 7, 9
    ...
    @testset verbose=true "FTField: norm and norm-difference identities                " begin
    ...
    @testset verbose=true "VectorField: dot decomposes component-wise                  " begin
    ...
    @testset verbose=true "normdiff with shifts == norm of explicitly shifted diff     " begin
        # `normdiff(u, v, shifts)` is documented as `norm(u - shift(v, shifts))`.
        # Verify by computing both sides on a rectangular grid with two shifts.
        ...
        # The grid has two homogeneous dimensions. Reject short and long
    ...
    @testset verbose=true "minnormdiff follows the number of transform dimensions      " begin
        # A planar rectangular grid has one transform dimension, so N and the returned shift
        # tuple both have length 1.
        Nx, Ny = 7, 9
        g1 = planar_test_grid(Nx, Ny)
        u1 = FTField(g1, randn(ComplexF64, Nx, (Ny>>1)+1))
        diff1, shifts1 = minnormdiff(u1, u1, (4,))
        @test diff1 ≈ 0 atol=1e-14
        @test shifts1 == (0.0,)

        # A channel-like grid has two transform dimensions, so the candidate grid and
        # returned shift tuple both have length 2.
        Ny2, Nx2, Nz2 = 7, 9, 5
        g2 = channel_test_grid(Ny2, Nx2, Nz2)
    ...
    @testset verbose=true "ProjectedField: dot is real and matches the explicit formula" begin
    ...
    @testset verbose=true "FTField dot matches HCubature on RectangularGrid            " begin
        # RectangularGrid uses the generic dot from norms.jl. With a function
        # independent of y, the
        # formula reduces to:
        #
        #   dot(û, û) = sum(ws) × (1/(Lx·Lz)) × ∫₀^{Lx}∫₀^{Lz} u(x, z)² dx dz
        #
        # We verify this against HCubature for a smooth periodic function.
        Ny, Nx, Nz = 7, 65, 65
        α, β = 1.0, 1.0
        g = channel_test_grid(Ny, Nx, Nz; α, β)
        Lx, Lz = 2π / α, 2π / β
        û = NSEBase.FFT(NSEBase.Field(g, (x, _, z, _) -> exp(sin(x)) * cos(z)))
        expected, _ = HCubature.hcubature(xz -> exp(2sin(xz[1])) * cos(xz[2])^2,
                                          [0.0, 0.0], [Lx, Lz])
        @test dot(û, û) ≈ sum(weights(g)) * expected / (Lx * Lz) rtol=1e-5
    end
end
