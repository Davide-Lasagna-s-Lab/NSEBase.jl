...
@testset verbose=true "FarazmandWeight                                             " begin
    ...
    @testset verbose=true "constructors and scale storage                              " begin
    ...
    @testset verbose=true "getindex evaluates the formula at a signed wavenumber       " begin
    ...
    @testset verbose=true "lmul! scales every coefficient by A[k] in place             " begin
        ...
        Ny, Nx, Nz = 7, 8, 4
        α, β = 2π, 2π
        g = unchecked_rectangular_test_size(channel_test_grid(Ny, 9, 5; α, β), Val((Ny, Nx, Nz)))
        Nm = 5
    ...
    @testset verbose=true "weighted dot(a, A, b) matches manual weighting              " begin
        ...
        Ny, Nx, Nz = 7, 8, 4
        g = unchecked_rectangular_test_size(channel_test_grid(Ny, 9, 5; α=2π, β=2π), Val((Ny, Nx, Nz)))
        Nm = 4
        Ψ = ntuple(_ -> randn(ComplexF64, Ny, Nm, (Nx>>1)+1, Nz), 3)
        a = ProjectedField(g, randn(ComplexF64, Nm, (Nx>>1)+1, Nz), Ψ)
