...
@testset verbose=true "Galerkin projection and expansion                           " begin
    @testset verbose=true "explicit formulas                                           " begin
        Ny, Nx, Nm, Nc = 7, 5, 2, 2
        Nk = (Nx >> 1) + 1
        ws = [0.25, 0.5, 1.25, 0.75, 1.0, 1.5, 0.4]
        g = planar_test_grid(Ny, Nx; ws)
    ...
    @testset verbose=true "LoopGalerkin and GemmGalerkin agree                         " begin
        Ny, Nx, Nm, Nc = 7, 7, 3, 2
        Nk = (Nx >> 1) + 1
        ws = [1.0, 0.5, 1.5, 2.0, 0.75, 1.25, 0.4]
        g = planar_test_grid(Ny, Nx; ws)
    ...
    @testset verbose=true "project-expand round trip                                   " begin
        Ny, Nx, Nc = 7, 5, 3
        Nk = (Nx >> 1) + 1
        g = planar_test_grid(Ny, Nx; ws=ones(Ny))
