...
#
# These tests pin the contract documented in `src/grids/abstractgrid.jl`:
#
...
@testset verbose=true "AbstractGrid interface                                      " begin
    ...
    struct IncompleteGrid{T, D, AXES, FFT_DIMS, S} <: AbstractGrid{T, D, AXES, FFT_DIMS} end
    Base.size(::IncompleteGrid{T, D, AXES, FFT_DIMS, S}) where {T, D, AXES, FFT_DIMS, S} = S
    bare_grid() = IncompleteGrid{Float64, 3, (2, 1, 3, nothing), (2, 3), (4, 8, 6)}()


    @testset verbose=true "type-parameter accessors                                    " begin
        g = bare_grid()
        ...
        time_spectral = IncompleteGrid{Float64, 4, (2, 1, 3, 4), (2, 3, 4), (4, 8, 6, 10)}()
        @test spatial_fft_storage_dims(time_spectral) === (2, 3)
        ...
        space_time_inhomogeneous = IncompleteGrid{Float64, 4, (1, 2, 3, 4), (1, 2), (8, 6, 4, 10)}()
        @test inhomogeneous_storage_dims(space_time_inhomogeneous) === (3, 4)
        @test spatial_inhomogeneous_storage_dims(space_time_inhomogeneous) === (3,)
    ...
    @testset verbose=true "transform_size: rfft halves only ORDER[1]                   " begin
    ...
    @testset verbose=true "fft_norm: product over FFT dims                             " begin
    ...
    @testset verbose=true "to_storage_order: AXES storage order                        " begin
        ...
        perm_grid = IncompleteGrid{Float64, 4, (2, 1, 3, 4), (2, 3, 4), (4, 8, 6, 10)}()

        @test NSEBase.to_storage_order((:A, :B, :C, :D), perm_grid) ===
              (:B, :A, :C, :D)

        # A grid without time drops the fourth logical coordinate while keeping
        # the stored dimensions in array order.
        @test NSEBase.to_storage_order((:X, :Y, :Z, :T), bare_grid()) ===
              (:Y, :X, :Z)
        ...
        space_time = IncompleteGrid{Float64, 3, (2, 1, nothing, 3), (2, 3), (4, 8, 10)}()
        @test NSEBase.to_storage_order((:X, :Y, :Z, :T), space_time) ===
              (:Y, :X, :T)

        # Argument count must match the four Cartesian coordinate slots.
        @test_throws MethodError NSEBase.to_storage_order((:A, :B), perm_grid)
    end

    @testset verbose=true "index utility functions                                     " begin
    ...
    @testset verbose=true "required interface fallthroughs throw                       " begin
        # Methods that have no default implementation must throw
        # `NotImplementedError` when called on a grid that doesn't override
        # them — this is the explicit contract from `grids/abstractgrid.jl`.
        g = bare_grid()
    ...
    @testset verbose=true "RectangularGrid satisfies the interface                     " begin
        # Round-trip every interface method on a production concrete grid.
        # This grid is 2-D with AXES=(1,2,nothing,nothing), ORDER=(2,) — i.e.
