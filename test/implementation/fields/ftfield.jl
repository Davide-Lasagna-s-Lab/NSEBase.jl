...
@testset verbose=true "FTField contract                                            " begin
    ...
    @testset verbose=true "constructor normalises mean and Hermitian mirror pairs      " begin
        Ny, Nx, Nz = 7, 9, 5
    ...
    @testset verbose=true "growto copies source coefficients to the same wavenumbers   " begin
        Ny, Nx, Nz = 7, 9, 5
        g  = channel_test_grid(Ny, Nx, Nz)
        u  = FTField(g)
        ...
        # Grow to a larger grid: (Nx, Nz) → (17, 11)
        v = NSEBase.growto(u, (17, 11))

        @test size(grid(v)) == (Ny, 17, 11)
        @test size(parent(v)) == (Ny, 9, 11)   # rfft: (17>>1)+1=9, Nz stays 11
    ...
    @testset verbose=true "homogeneous_axes and inhomogeneous_axes                     " begin
        Ny, Nx, Nz = 7, 9, 5
        g  = channel_test_grid(Ny, Nx, Nz)   # FFT_DIMS_ORDER = (2, 3)
    ...
    @testset verbose=true "WaveNumberVector getindex and setindex preserve symmetry    " begin
        Ny, Nx, Nz = 7, 9, 5
        g = channel_test_grid(Ny, Nx, Nz)
        u = FTField(g)
