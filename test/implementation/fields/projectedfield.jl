...
@testset verbose=true "ProjectedField contract                                     " begin
    ...
    @testset verbose=true "constructor uses mode axis followed by FFT axes             " begin
        Ny, Nx, Nz, Nm = 7, 9, 5, 4
    ...
    @testset verbose=true "linear indexing (a[i]) reads and writes the parent array    " begin
        Ny, Nx, Nz, Nm = 7, 5, 3, 2
        g = channel_test_grid(Ny, Nx, Nz)
    ...
    @testset verbose=true "storage indexing (a[m,i1,i2,...]) matches parent array      " begin
        Ny, Nx, Nz, Nm = 7, 5, 3, 2
        g = channel_test_grid(Ny, Nx, Nz)
        a = ProjectedField(g, _projected_modes(g, Nm))
    ...
    @testset verbose=true "CartesianIndex indexing matches storage indexing            " begin
        Ny, Nx, Nz, Nm = 7, 5, 3, 2
        g = channel_test_grid(Ny, Nx, Nz)
        a = ProjectedField(g, _projected_modes(g, Nm))
    ...
    @testset verbose=true "WaveNumberVector indexing preserves modal symmetry          " begin
        Ny, Nx, Nz, Nm = 7, 9, 5, 3
        g = channel_test_grid(Ny, Nx, Nz)
        a = ProjectedField(g, _projected_modes(g, Nm))
    ...
    @testset verbose=true "WaveNumberVector get reads conjugate for negative rfft index" begin
        Ny, Nx, Nz, Nm = 7, 9, 5, 2
        g = channel_test_grid(Ny, Nx, Nz)
        a = ProjectedField(g, _projected_modes(g, Nm))
    ...
    @testset verbose=true "copy and zero return independent fields with same grid/modes" begin
        Ny, Nx, Nz, Nm = 7, 5, 3, 2
        g = channel_test_grid(Ny, Nx, Nz)
        Ψ = _projected_modes(g, Nm)
        a = ProjectedField(g, Ψ)
    ...
    @testset verbose=true "abs returns element-wise absolute values                    " begin
        Ny, Nx, Nz, Nm = 7, 5, 3, 2
        g = channel_test_grid(Ny, Nx, Nz)
        a = ProjectedField(g, _projected_modes(g, Nm))
    ...
    @testset verbose=true "constructor enforces DC-plane mean and Hermitian symmetry   " begin
    ...
    @testset verbose=true "apply_symmetry! only modifies the DC (rfft=0) plane         " begin
        Ny, Nx, Nz, Nm = 7, 9, 5, 4
        g  = channel_test_grid(Ny, Nx, Nz)
        Ψ  = _projected_modes(g, Nm)
        a  = ProjectedField(g, Ψ)
    ...
    @testset verbose=true "apply_symmetry! is idempotent                               " begin
        Ny, Nx, Nz, Nm = 7, 9, 5, 4
        g = channel_test_grid(Ny, Nx, Nz)
        a = ProjectedField(g, _projected_modes(g, Nm))
        parent(a) .= randn(ComplexF64, size(parent(a)))
    ...
    @testset verbose=true "apply_symmetry! handles all modes uniformly                 " begin
