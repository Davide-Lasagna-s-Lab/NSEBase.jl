@testset verbose=true "Field                                                       " begin
    ...
    @test begin
        ...
        for nx in 1:16, ny in 1:11
            u1[nx, ny] != fun(x[nx], y[ny], nothing, nothing) && (res = false; break)
        end; res
    end
    @test begin
        res = true
        for nx in 1:16, ny in 1:17
            u2[nx, ny] != fun(xd[nx], yd[ny], nothing, nothing) && (res = false; break)
        end; res
    end
...
@testset verbose=true "Hermitian symmetry                                          " begin
    # 1 dimension
...
@testset verbose=true "VectorField                                                 " begin
    ...
    f1(x, y, _, _)=x^2*cos(2π*y/L)
    f2(x, y, _, _)=x^3*sin(2π*y/L)
    f3(x, y, _, _)=exp(x)*exp(cos(2π*y/L))
    u1 = VectorField(g)
    ...
    u = VectorField(FTField(g, 0.5*ones(ComplexF64, Nx, (Ny >> 1) + 1)),
                    ...
                    FTField(g, 0.5*ones(ComplexF64, Nx, (Ny >> 1) + 1)))
    @test dot(u, u) ≈ 3 * 0.25 * Ny * sum(weights(g))
