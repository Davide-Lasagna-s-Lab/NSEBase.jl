...
@testset verbose=true "Shifts                                                      " begin
    ...
    @testset verbose=true "FTField: phase factor matches the analytic shift            " begin
        # Use a channel-like rectangular grid:
        #   dim 1: inhomogeneous y (length Ny)
        #   dim 2: rfft x ∈ [0, 2π/α)
        #   dim 3: signed FFT z ∈ [0, 2π/β)
        Ny, Nx, Nz = 7, 8, 6
        α, β = 1.0, 1.0
        g = unchecked_rectangular_test_size(channel_test_grid(Ny, 9, 7; α, β), Val((Ny, Nx, Nz)))
    ...
    @testset verbose=true "zero shift is identity and touches no memory                " begin
    ...
    @testset verbose=true "VectorField: shift! applies component-wise                  " begin
    ...
    @testset verbose=true "shift returns a new field, leaves the original intact       " begin
    ...
    @testset verbose=true "two shifts compose additively                               " begin
    ...
    @testset verbose=true "ProjectedField: shift! changes only the homogeneous indices " begin
