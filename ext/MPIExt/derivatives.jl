...
function NSEBase.laplacian!(out::DecomposedFTField,
    ...
    # initialise task for boundary work and immediately return
    boundary_tasks(NSEBase.grid(out))[dim] = Threads.@spawn begin
...
function dd!(out::F,
    ...
    F<:Union{FTField{G}, Field{G}}}

    isnothing(STORAGE_DIM) && return out
    if STORAGE_DIM ∉ FFT_DIMS_ORDER && STORAGE_DIM ∈ DDIMS
        _distributed_dd!(out, u, Val(STORAGE_DIM); adjoint=adjoint)
    else
        NSEBase.dd!(out, u, Val(STORAGE_DIM); adjoint=adjoint)
    end
...
function _distributed_dd!(out::Union{DecomposedFTField, DecomposedField},
    ...
    boundary_tasks(NSEBase.grid(out))[dim] = Threads.@spawn begin
        NSEBase.wait_requests(requests)
        boundary_boundary!(out, u, Val(STORAGE_DIM); adjoint=adjoint)
    end
