...
Call `f(one_or_two, i_H1, i_H2, …, i_HN)` for every stored spectral mode of a
...
- `one_or_two`: `1` if `i_H1 == 1` (zero rfft wavenumber, no conjugate partner),
  `2` otherwise (mode represents both `+k` and `-k` via Hermitian symmetry).
...
Inner product of two `ProjectedField`s. Modes with `one_or_two == 2` represent
...
Use this instead of composing `for_each_mode` with a manual inhomogeneous loop
