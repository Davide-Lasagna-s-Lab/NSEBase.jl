...
Call `f(n_H1, n_H2, …, n_HN)` for every stored spectral wavenumber of a field on
...
Prefer `for_each_freq` over `for_each_wavenumber` when `f` needs to reason about
wavenumber values directly — for example, to look up the same wavenumber in two
grids of different sizes, or to apply a wavenumber-dependent filter.  Use
`for_each_wavenumber` when only storage indices are needed (e.g. inner products,
in-place scaling).

# Example

Zero-pad (or truncate) a spectral field onto a larger (or smaller) grid by
copying every wavenumber that exists in both grids.  `_combine_indices`
maps a `WaveNumberVector` (wavenumber tuple) to the correct storage index on a
...
function growto(u::FTField, target_size)
    ...
    for_each_freq(grid(u)) do args...
        idx_src = _combine_indices(grid(u),   I, WaveNumberVector(args...))
        idx_dst = _combine_indices(grid(out), I, WaveNumberVector(args...))
...
(as yielded by `for_each_wavenumber`) to a tuple of signed integer wavenumbers.
