...
#
# Provides thin wrappers around JLD2 file I/O for the two objects that need to
# be saved and loaded between runs: the computational grid and the modal
# coefficients of a `ProjectedField`.
#
# Grids are saved and loaded as whole objects under the key `"grid"`.  They
# must be serialisable by JLD2 (concrete structs with plain-data fields
# satisfy this automatically).
#
# `ProjectedField` data is saved as the raw coefficient array under `"data"`,
# without the modes or grid.  Loading requires the caller to supply a matching
# grid and modes, which avoids duplicating potentially large mode arrays on
# disk while keeping the load interface explicit about the basis being used.
