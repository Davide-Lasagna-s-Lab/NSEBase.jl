...
# `wavenumber_scale`, `weights`). Most other grid-aware behaviour—including
# spectral loop generation, transform sizing, base-flow injection, and Fourier
# derivatives—is derived automatically from the compile-time type parameters.
# Grids with non-Fourier directions additionally provide the finite-difference
# hooks documented on `AbstractGrid`.
#
...
Domain decomposition is deliberately not encoded in these type parameters.
MPI integrations wrap a serial grid in a decomposed-grid object that owns the
partition metadata and delegates geometric queries and finite-difference
stencils to its parent grid. A concrete grid therefore describes the same
physical discretisation in serial and distributed runs and need not know how
...
- `weights(grid)` returning quadrature weights for the inhomogeneous
  dimensions.
- `inhomogeneous_dd!(out, u, Val(dim); adjoint=false)` for first derivatives
  along each non-Fourier storage dimension.
- `inhomogeneous_laplacian!(out, u; adjoint=false)` for the sum of spatial
...
- `Base.eltype(grid)`: returns the scalar type parameter `T`.
...
"""Return the real scalar type encoded by an [`AbstractGrid`](@ref)."""
...
coordinates, i.e. `fft_storage_dims(grid)` with the temporal dimension excluded.

When the logical time coordinate is absent, this is the same tuple as
[`fft_storage_dims`](@ref). If a grid carries a transformed logical time
coordinate—even with temporal resolution one—that dimension is omitted.
"""
...
# Coordinate symbols are the deliberate public representation of physical
# directions. The `Val{:x}` forms below retain compile-time dispatch without
# introducing a second enum-based vocabulary that callers would need to convert.
...
The direction may be `:x`, `:y`, `:z`, or `:t`, or `Val` of one of those
symbols. A direction absent from the grid returns `nothing`.
...
For any periodic coordinate of length `L` the factor is `2π/L`. A unit-period
temporal direction therefore returns `2π`.
