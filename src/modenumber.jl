...
# fft_dims(g) = (Hs..., Ht).  The first wavenumber (ns[1]) is for the rfft
# dimension H[1]; negative values trigger conjugate-symmetry lookup.  All
...
"""
    _modenumber_to_projected_indices(g::AbstractGrid, n::ModeNumber{N}) where {N}
...
`ProjectedField` array (axes follow the order of `fft_dims(g) = (Hs…, Ht)`).
...
"""
function _modenumber_to_projected_indices(g::AbstractGrid, n::ModeNumber{N}) where {N}
    H = fft_dims(g)           # e.g. (2, 3, 1) for channel flow
    if n.ns[1] >= 0
