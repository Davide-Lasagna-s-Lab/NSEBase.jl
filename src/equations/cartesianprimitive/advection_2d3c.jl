...
function adjoint_advection!(out, v::VectorField{3, F}, eq::CartesianPrimitiveNSE{2, 3}, ::Advective, ::AdjointContinuous) where {F<:FTField}
    dudx = eq.scache[1]; dvdx = eq.scache[2]; dvdy = eq.scache[3]
    U    = eq.pcache[1]; dUdx = eq.pcache[2]; dUdy = eq.pcache[3]
    V    = eq.pcache[4]; dVdx = eq.pcache[5]; dVdy = eq.pcache[6]
...
function adjoint_advection!(out, v::VectorField{3, F}, eq::CartesianPrimitiveNSE{2, 3}, ::Divergence, ::AdjointContinuous) where {F<:FTField}
    U = eq.pcache[1]; dUdx = eq.pcache[2]; dUdy = eq.pcache[3]
...
adjoint_advection!(out, v::VectorField{3, F}, eq::CartesianPrimitiveNSE{2, 3}, ::Rotational, ::AdjointContinuous) where {F<:FTField} =
...
# ================================================================== #
function adjoint_advection!(out, v::VectorField{3, F}, eq::CartesianPrimitiveNSE{2, 3}, ::Advective, ::AdjointDiscrete) where {F<:FTField}
