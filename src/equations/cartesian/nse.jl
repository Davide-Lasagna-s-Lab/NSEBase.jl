# Velocity-only Cartesian Navier-Stokes operator.
#
...
"""
    CartesianNSE{NDIM, NCOMP, MODE, FORM, V, FFT, S, P, BF}

Velocity-only Cartesian Navier-Stokes operator. `NDIM`/`NCOMP` fix the
...
mutable struct CartesianNSE{NDIM, NCOMP, MODE<:Mode, FORM<:AdvectionForm, V, FFT, S, P, BF} <: AbstractNSE{MODE, FORM}
    ...
    CartesianNSE{NDIM, NCOMP, MODE, FORM}(visc::V, plans::FFT,
